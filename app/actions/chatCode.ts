...
export async function chatCode(
  ...
  try {
    ...
    const contents = [
      {
        role: "user",
        parts: [{ text: `You are an expert pair-programmer. I am looking at the following code which supposedly has some issues: \n\n\`\`\`\n${contextCode}\n\`\`\`\n\nI will ask you questions about it.` }]
      },
      {
        role: "model",
        parts: [{ text: "Understood. I have the code context. How can I help you debug or explain it?" }]
      },
      ...history.map(msg => ({
        role: msg.role,
